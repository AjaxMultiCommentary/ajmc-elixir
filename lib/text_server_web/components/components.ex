defmodule TextServerWeb.Components do
  ...
  def card(assigns) do
    ...
    <div class="group relative">
      <div class="w-full min-h-80 bg-primary aspect-w-1 aspect-h-1 rounded-md overflow-hidden group-hover:opacity-75 lg:h-80 lg:aspect-none">
      ...
      <div class="mt-4 flex justify-between">
        <div>
          <h3 class="font-bold base-content">
            <a href={@url}>
              <span aria-hidden="true" class="absolute inset-0"></span> <%= Map.get(@item, :title) %>
            </a>
          </h3>
          <p class="mt-1 text-sm base-content"><%= @item.description %></p>
        </div>
      </div>
    </div>
    """
    ...
    <div>
      ...
      >
        ...
        <summary class="collapse-title">
          <h3 class="text-md font-medium leading-6 base-content">
            <%= c.attributes |> Map.get("lemma") %>
          </h3>
          <small class="mt-1 mx-w-2xl text-sm base-content"><%= c.author %></small>
        </summary>
        <div class="collapse-content float-right">
          <p class="mt-1 max-w-2xl text-sm base-content"><%= c.content %></p>
        </div>
      </details>
    </div>
    """
  ...
  def search_list(assigns) do
    ~H"""
    <ul role="list" class="divide-y divide-primary sm:max-w-lg">
      <%= for item <- @items do %>
    ...
    <a href={@url}>
      <div class="flex py-6 rounded shadow-md hover:shadow-lg">
        <div class="ml-4 flex flex-1 flex-col">
          <div>
            <div class="flex justify-between text-base font-medium base-content">
              <h3><%= @title %></h3>
            </div>
          </div>
          <div class="flex flex-1 justify-between text-sm">
            <p class="text-sm base-content"><%= @description %></p>
          </div>
        </div>
      </div>
    </a>
    ...
    <div class="flex items-center justify-between border-t border-stone-200 bg-white px-4 py-3 sm:px-6">
      <div class="sm:flex sm:justify-between mx-auto">
        <nav class="isolate inline-flex -space-x-px rounded-md shadow-sm" aria-label="Pagination">
          ...
          <!-- Current: "z-10 bg-stone-50 border-stone-500 text-stone-600", Default: "bg-white border-primary base-content hover:bg-primary-50" -->
  ...
  defp first_page_button(assigns) do
    ...
    <.link
      ...
      class={[
        ...
        "font-medium",
        "text-stone-100",
        if(@current_page == 1, do: "cursor-default", else: "hover:bg-primary-50 text-stone-700")
      ]}
    >
  ...
  defp last_page_button(assigns) do
    ...
    <.link
      ...
      class={[
        ...
        "font-medium",
        "base-content",
        if(@current_page == @total_pages, do: "cursor-default", else: "hover:bg-primary-50 text-stone-700")
      ]}
    >
  ...
  defp numbered_button_classes(current_page, i) do
    [
      ...
      if(current_page == i,
        ...
        else: "base-content hover:bg-primary-50"
      )
    ]
  end
