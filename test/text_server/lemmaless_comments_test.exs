defmodule TextServer.LemmalessCommentsTest do
  ...
  describe "lemmaless_comments" do
    ...
    test "create_lemmaless_comment/1 with valid data creates a lemmaless_comment" do
      valid_attrs = %{attributes: %{}, content: "some content", urn: %{}}

      assert {:ok, %LemmalessComment{} = lemmaless_comment} = LemmalessComments.create_lemmaless_comment(valid_attrs)
      assert lemmaless_comment.attributes == %{}
      assert lemmaless_comment.content == "some content"
      assert lemmaless_comment.urn == %{}
    end

    test "create_lemmaless_comment/1 with invalid data returns error changeset" do
      assert {:error, %Ecto.Changeset{}} = LemmalessComments.create_lemmaless_comment(@invalid_attrs)
    end

    test "update_lemmaless_comment/2 with valid data updates the lemmaless_comment" do
      lemmaless_comment = lemmaless_comment_fixture()
      update_attrs = %{attributes: %{}, content: "some updated content", urn: %{}}

      assert {:ok, %LemmalessComment{} = lemmaless_comment} = LemmalessComments.update_lemmaless_comment(lemmaless_comment, update_attrs)
      assert lemmaless_comment.attributes == %{}
    ...
    test "update_lemmaless_comment/2 with invalid data returns error changeset" do
      lemmaless_comment = lemmaless_comment_fixture()
      assert {:error, %Ecto.Changeset{}} = LemmalessComments.update_lemmaless_comment(lemmaless_comment, @invalid_attrs)
    ...
    test "delete_lemmaless_comment/1 deletes the lemmaless_comment" do
      lemmaless_comment = lemmaless_comment_fixture()
      assert {:ok, %LemmalessComment{}} = LemmalessComments.delete_lemmaless_comment(lemmaless_comment)
      assert_raise Ecto.NoResultsError, fn -> LemmalessComments.get_lemmaless_comment!(lemmaless_comment.id) end
    end
